...
class MiniHackNetwork(nn.Module):
    def __init__(self, processed_observation_shape, config):
        super().__init__()
        ...
        self.k_dim = config.embedding_dim
        self.h_dim = config.hidden_dim

        self.crop_model = config.crop_model
        ...
        self.glyph_type = config.glyph_type
        self.glyph_embedding = RLLibGlyphEmbedding(
            config.glyph_type,
            config.embedding_dim,
            None,
            config.use_index_select,
        )

        K = config.embedding_dim  # number of input filters
        F = 3  # filter dimensions
        S = 1  # stride
        P = 1  # padding
        M = 16  # number of intermediate filters
        self.Y = 8  # number of output filters
        L = config.layers  # number of convnet layers
        ...
        # MESSAGING MODEL
        if config.msg_model is None:
            self.msg_model = "none"
        else:
            self.msg_model = config.msg_model
            self.msg_hdim = config.msg_hidden_dim
            self.msg_edim = config.msg_embedding_dim
        ...
        if self.msg_model.endswith("cnn"):
            ...
            if self.msg_model == "cnn":
                ...
                raise NotImplementedError("msg_model == %s", config.msg_model)
            ...
            raise NotImplementedError("_model == %s", config.msg_model)
        ...
        self.equalize_input_dim = config.equalize_input_dim
        if not self.equalize_input_dim:
            ...
            project_hdim = config.equalize_factor * self.h_dim
