...
all_scores = {
    "AutoAscend-Bot": np.array(aligned_scores).transpose(),
    "Zero": np.array(aligned_scores).transpose() / 4,
    "Half": np.array(aligned_scores).transpose() / 2,
}
...
aggregate_func = lambda x: np.array(
    [
        ...
        # metrics.aggregate_optimality_gap(x, gamma=56),
    ]
)
aggregate_scores, aggregate_score_cis = rly.get_interval_estimates(
    all_scores, aggregate_func, reps=5
)
...
fig, axes = plot_utils.plot_interval_estimates(
    ...
    xlabel="Maximum Level Reached",
    xlabel_y_coordinate=-1.16 / 2.0,
)
for ax in axes.reshape(-1):
    ax.set_xlim([0, 4])
plt.savefig("reliable.png", bbox_inches="tight")

thresholds = np.linspace(0.0, 20, 40)
...
plot_utils.plot_performance_profiles(
    ...
    xlabel=r"Maximum Level Reached $(\tau)$",
    ax=ax,
)
