...
def cql_dqn_loss(
    ...
    with torch.no_grad():
        next_q_values, next_target_rnn_states = target_critic(
            next_obs, state=target_rnn_states
        )
    ...
    return total_loss, next_rnn_states, next_target_rnn_states, loss_info


class Critic(nn.Module):
    def __init__(self, action_dim, rnn_hidden_dim, rnn_layers, use_prev_action=True):
        super().__init__()
    ...
    def forward(self, inputs, state=None):
        ...
        q_values_ensemble = self.head(core_output).view(B, T, self.num_actions)
    ...
    def act(self, obs, state=None, device="cpu"):
        inputs = {
            "screen_image": torch.tensor(obs["screen_image"], device=device)[
                None, None, ...
            ],
            "tty_chars": torch.tensor(obs["tty_chars"], device=device)[None, None, ...],
            "prev_actions": torch.tensor(
                obs["prev_actions"], dtype=torch.long, device=device
            )[None, None, ...],
        }
...
def train(config: TrainConfig):
    ...
    for step in trange(config.update_steps, desc="Training"):
        ...
        loss, rnn_state, target_rnn_state, loss_info = cql_dqn_loss(
            critic=critic,
        ...
        wandb.log(
            dict(
                loss_info, **{"transitions": config.batch_size * config.seq_len * step}
            ),
            step=step,
        )
        ...
        if (step + 1) % config.eval_every == 0:
            eval_stats = evaluate(
                env_builder, critic, config.eval_episodes_per_seed, device=DEVICE
            )
            wandb.log(
                dict(
                    eval_stats,
