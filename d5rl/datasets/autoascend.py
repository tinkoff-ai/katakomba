...
class _AutoAscendTTYIterator:
    ...
    def __iter__(self):
        # Initialize buffers
        self._states  = np.zeros((self._batch_size, self._n_prefetched_batches, 24, 80, 3))
    ...
    def __next__(self):
        ...
        for b_ind in range(self._batch_size):
            cur_ind      = self._2d_indices[self._indices[self._cur_ind % self._total_samples]]
            batch_ind    = cur_ind[0]
            prefetch_ind = cur_ind[1]

            # for seq_ind in range(self._seq_len):                
            b_s [b_ind] = self._states[batch_ind, prefetch_ind : prefetch_ind + self._seq_len]
            b_a [b_ind] = self._actions[batch_ind, prefetch_ind : prefetch_ind + self._seq_len]
            b_r [b_ind] = self._rewards[batch_ind, prefetch_ind : prefetch_ind + self._seq_len]
            b_d [b_ind] = self._dones[batch_ind, prefetch_ind : prefetch_ind + self._seq_len]
            b_ns[b_ind] = self._states[batch_ind, prefetch_ind + 1 : prefetch_ind + 1 + self._seq_len]
    ...
    def _init_prefetch(self):
        ...
        self._2d_indices    = list(itertools.product(range(self._batch_size), range(self._n_prefetched_batches - self._seq_len)))
        self._total_samples = len(self._2d_indices)
        self._indices       = np.random.choice(
            a       = range(self._total_samples),
